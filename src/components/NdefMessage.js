...
class NdefMessage extends React.Component {
  render() {
    ...
    const tnfName = tnfValueToName(ndef.tnf); //WELL_KNOWN
    const rtdName = rtdValueToName(ndef.type); //URI
...
class RtdUriPayload extends React.Component {
  ...
  render() {
    let {ndef} = this.props;
    let uri = Ndef.uri.decodePayload(ndef.payload); // decode the  value
    return (
